...
class GPTBetFoot:
    ...
    def parse_text_content(self, content):
        """Parse le contenu texte sans pandas"""
        data = {
    ...
    def calculate_fair_odds(self, data):
        """Calcule les cotes fair sans numpy"""
        try:
            # Extraction données avec valeurs par défaut
            ...
            # Calcul du score (remplacement de numpy)
            delta_xg = xg_home - xg_away
            delta_rank = (20 - rank_home) - (20 - rank_away)  # Meilleur classement = score plus élevé
            delta_form = form_home - form_away
            
            score = 0.3 * delta_xg + 0.2 * delta_rank + 0.1 * delta_form
            
            # Fonction exponentielle manuelle (remplacement de numpy.exp)
            def exp(x):
                return 2.71828 ** x
            
            # Probabilités logistiques
            p_home = 1 / (1 + exp(-score))
    ...
    def simulate_trade(self, recommendation):
        """Simule un trade avec résultat aléatoire"""
    ...
    def get_performance_stats(self):
        """Calcule les statistiques de performance sans pandas"""
        if not self.trades:
            return {
                'total_trades': 0,
                'winning_trades': 0,
...
def reset():
    """Reset le système (pour les tests)"""
    bot.bankroll = 10000
    bot.trades = []
    return jsonify({'message': 'Système réinitialisé', 'bankroll': bot.bankroll})
